...
public class AgilePLMServiceImpl {
	 ...
	 * @throws APIException
	 */
	public AttachmentBean getAttachment(String docNumber, String fileName, String fileDesc)
...
//default file desc
		if (fileName != null){
			if(!attCriteria.toString().isEmpty())
				attCriteria.append(" AND ");
		...
		if (finalAttachment != null) {
			...
			attBean.setAttName(finalAttachment.getValue("filename").toString());
			attBean.setAttStream(((IAttachmentFile) finalAttachment).getFile());
